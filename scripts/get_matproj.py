...
def get_all_results():
    with pymatgen.ext.matproj.MPRester(MATPROJ_API_KEY) as r:
        ...
        print(f'Number of materials: {len(material_ids)}')
        chunk_size = 10000
        sublists = [
...
def get_all_atoms():
    ...
    for res in get_all_results():
        ...
        property_list = [structure, delta_e, mp_id, e_above_hull,
            band_gap, sg_number, sg_symbol, 
            sg_pointgroup, sg_crystal_system]
...
def main():
    ...
    print("Writing to DB")
    with ase.db.connect(
        os.path.join("matproj", "matproj.db"), append=False
    ) as db:
