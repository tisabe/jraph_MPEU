...
def dict_to_ase(aflow_dict):
    ...
    cell = str_to_array(aflow_dict['relaxed_cell'])
    positions = str_to_array(aflow_dict['relaxed_atom_positions'])
    symbols = aflow_dict['compound_name']
    structure = Atoms(
        symbols=symbols,
...
def main(args):
    ...
    icsd_col = aflow_df['ICSD_number']
    logging.info("Length of dataframe: ", len(icsd_col))
