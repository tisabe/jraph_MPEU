...
class TestModelFunctions(unittest.TestCase):
    ...
    def test_random_graph_batch(self):
        ...
        net_fn = MPEU_uq(self.config, True)
        net = hk.transform(net_fn)
        params = net.init(init_rng, graph)

        graph_pred = net.apply(params, rng, graph)
        prediction = graph_pred.globals
        self.assertTupleEqual(prediction['mu'].shape, (n_graphs, 1))
        self.assertTupleEqual(prediction['sigma'].shape, (n_graphs, 1))
        np.testing.assert_array_less([[1e-6]]*n_graphs, prediction['sigma'])
